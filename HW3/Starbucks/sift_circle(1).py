...
def drawRetangle(img1, img2, src_pts, dst_pts, matchesMask,sx1,sy1,sx2,sy2):
    ...
    # cv2.rectangle(img3,(int(point1[0]),int(point1[1])),(int(point4[0]),int(point4[1])),(0, 0, 255))
    # cv2.imwrite(file_name+'rectangle2.jpg', img3) 

def circleDetection(file_name):
    img = cv2.imread(file_name+'.jpg',0)
    img = cv2.medianBlur(img,5)
    cimg = cv2.cvtColor(img,cv2.COLOR_GRAY2BGR)
    dis = min(img.shape[0],img.shape[1])
    circles = cv2.HoughCircles(img,cv2.cv.CV_HOUGH_GRADIENT,1,dis/3,
                                param1=50,param2=20,minRadius=30,maxRadius=300)
...
def test(input1, input2):
    ...
    for i in circles[0,:]:
        ...
        if len(good)>MIN_MATCH_COUNT:
            ...
            try:
                ...
                points.append(drawRetangle(img1, img2_total, src_pts, dst_pts, matchesMask,0,0,sx,sy))
    ...
    average = total/found
    thredhold = 0.01*(best-average)+average

    found = 0
    for i in range(len(points)):
        if (good_num[i]>thredhold):
            cv2.rectangle(img3,(points[i][0],points[i][1]),(points[i][2],points[i][3]),(0, 255, 0),10)
            found += 1

    if (found<1):
        cv2.rectangle(img3,(points[best_index][0],points[best_index][1]),(points[best_index][2],points[best_index][3]),(0, 255, 0),10)
...
if __name__ == "__main__":
    processImage('template.gif')
    test('template-0','image1')
    test('template-0','image2')
    test('template-0','image3')
    test('template-0','image4')
    test('template-0','image5')
    test('template-0','image6')
    test('template-0','image7')
    test('template-0','image8')
    test('template-0','image9')
    test('template-0','image10')
    test('template-0','image11')
    test('template-0','image12')
    test('template-0','image13')
    test('template-0','image14')
    test('template-0','image15')
    test('template-0','image16')
